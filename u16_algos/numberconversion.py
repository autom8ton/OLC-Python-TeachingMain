...
def convert_storage(value, from_unit, to_unit):
    ...
    # will give value in bytes
    conversion_value = conversion_factors[from_unit] * value 

    # to complete the conversion, i need to divide by the source unit
...
while True:
    valuenum = float(input("Enter a number to convert: "))
...
while True:
    ...
    if is_valid_unit(from_unit):
        ...
        if is_valid_unit(to_unit):
            # if code comes here, means all valid
            output_val = convert_storage(valuenum, from_unit, to_unit)
            output_val = round(output_val, 4) # round to 4 decimal points
